  ...
  const getWhatsAppUrl = () => {
    const text =
      language === "si"
        ...
        : "Hi Madara Restaurant! I would like to contact you regarding catering & dine-in services.";
        ...
        <div className="flex items-center justify-between">
          ...
          <Link href="/" className="flex items-center gap-3 group">
            ...
            <div className="flex flex-col">
              ...
              <span className="text-[10px] tracking-widest uppercase font-bold text-[#f37b20]">
                Restaurant & Catering
              </span>
            </div>
          </Link>
              ...
              if (link.isCareer) {
                return (
                  <Link
                    key={link.href}
                    href={link.href}
                    className={`relative text-xs font-bold transition-colors uppercase tracking-wider ${
                      active
                        ? "text-[#f37b20] font-extrabold"
                        : "text-stone-800 hover:text-[#f37b20]"
                    }`}
                  >
                    <span>{language === "si" ? link.labelSi : link.labelEn}</span>
                    <span className="absolute -top-1 -right-2 flex h-2 w-2">
                      <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-[#f37b20] opacity-75"></span>
                      <span className="relative inline-flex rounded-full h-2 w-2 bg-[#f37b20]"></span>
                    </span>
                  </Link>
                );
              }
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            >
              <Globe className="w-3.5 h-3.5 text-[#f37b20]" />
            ...
            {/* WhatsApp Direct - Iconic #25D366 Green Pill Button */}
            <a
              href={getWhatsAppUrl()}
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-1.5 px-4 py-1.5 rounded-full text-xs font-bold bg-[#25D366] hover:bg-[#20ba59] text-white transition-all shadow-md hover:scale-105"
            >
              ...
              {navLinks.map((link) => {
                ...
                return (
                  <Link
                    ...
                    className={`relative inline-flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-bold transition-colors uppercase tracking-wider ${
                      active
